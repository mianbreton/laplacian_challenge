...
will be slow, because Nm1 is a long unsigned, while i,j,k are unsigned. 
Since unsigned < long unsigned, this is not a safe loop if Nm1 >= 2**32
/!\ g++ DOES NOT WARN ABOUT THAT /!\ 
It can warn only about signed vs unsigned.
Only solution: add the -Wconversion flag to compilation

Note that the following is fast

const size_t Nm1 = N-1ul;
for(uint32_t i=1ul; i < Nm1; i++)
for(uint32_t j(1ul); j < Nm1; j++)
#pragma omp simd
for(uint32_t k = 1ul; k < Nm1; k++)

Because of omp simd stuff...

However, even simd does not optimize when we have
const size_t Nm1 = N-1ul;
for(uint32_t i=1ul; i < Nm1; i++)
for(uint32_t j(1ul); j < Nm1; j++)
#pragma omp simd
...
instead of size_t idx. 

For some reason, full uint32_t is slow. Better to use size_t. Maybe due to the system. on 64-bit, the adressing is on 64 bits
const uint32_t N1 = N;
const uint32_t N2 = N*N;
const uint32_t Nm1 = N-1ul;
const float invh2 = static_cast<float>(N2);
